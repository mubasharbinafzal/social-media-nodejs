...
export const completeProfile = async (req, res) => {
  ...
  try {
    ...
    if (req.files.length > 5) {
      return res
        .status(400)
        .send({ success: false, message: "Kindly select max five images!" });
    }

    for (let file of req.files) {
      let image = new Image({
        imageTitle: process.env.BASE_URL + file.filename,
        userId: user._id,
      });
      await image.save();
    }
...
export const filterUsers = async (req, res) => {
  ...
  try {
    ...
    if (!filter) {
      return res
        .status(400)
        .send({ success: false, Message: "Kindly Set a filter" });
    }
    ...
    let users = [];
    if (gender && country && state && city && minAge && maxAge) {
      //filter users
      users = await User.find({
        ...
        state,
        city,
        $and: [{ age: { $lte: maxAge }, age: { $gte: minAge } }],
        $and: [{ block: false }, { deleted: false }],
      });

      //get images for all the users
      let allImages = [];
      ...
      //bind images to each user
      let modifiedUsers = users.map((user, index) => {
        return {
          _id: user._id,
          name: user.name,
          email: user.email,
...
export const updateFilter = async (req, res) => {
  try {
    ...
    let filterExist = await Filter.findOne({ userId });
    if (filterExist) {
      await Filter.updateOne({ userId }, req.body);
      return res
        .status(200)
        .send({ success: true, Message: "filter has been updated" });
    }
    let payload = req.body;
    payload.userId = userId;
